...
def get_tcgdex_sets(api_base):
    return fetch_json(f"{api_base}/en/sets") or []


def get_tcgdex_set_lang(api_base, lang, set_id):
    return fetch_json(f"{api_base}/{lang}/sets/{set_id}")
...
def upsert_translation(existing_translation, collection_id, lang_id, name):
    ...
    if existing_translation:
        return api_request(
            "PATCH",
            f"collection-translations/{existing_translation['id']}",
            payload
        ), False
...
def sync():
    ...
    settings_by_key = {
        item["setting_key"]: item.get("setting_value")
    ...
    collection_ids = {
        item["id"]
        for item in collections_by_code.values()
    }
    translations_by_collection_lang = get_existing_translations(
        translations,
        collection_ids
    )
    ...
    for i, item in enumerate(all_sets):
        ...
        for lang in api_lang:
            ...
            if lang == "en":
                ...
                tcgdex_translation = get_tcgdex_set_lang(
                    api_base,
                    lang,
                    set_id
                )
                name = (tcgdex_translation or {}).get("name", "")
                time.sleep(0.08)
