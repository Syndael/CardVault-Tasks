...
PARAM_KEY_API_BASE = "sync.yugioh.products.api.base"
PARAM_KEY_CAR_TYPE = "sync.yugioh.products.card.type"
...
def get_yugioh_card(api_base, passcode):
    data = fetch_json(f"{api_base.rstrip('/')}/cardinfo.php?id={urllib.parse.quote(str(passcode))}")
    if data and isinstance(data, dict) and data.get("data"):
        return data["data"][0]
    return None


def get_yugioh_card_lang(api_base, passcode, lang):
    url = f"{api_base.rstrip('/')}/cardinfo.php?id={urllib.parse.quote(str(passcode))}&language={urllib.parse.quote(lang)}"
    data = fetch_json(url)
    if data and isinstance(data, dict) and data.get("data"):
        return data["data"][0]
    return None
...
def get_yugioh_image_url(card_data):
    ...
    for i, product in enumerate(pending):
        ...
        try:
            ...
            for code in image_codes:
                ...
                try:
                    t = get_yugioh_card_lang(api_base, passcode, code)
                except Exception:
                    continue
                if t:
                    if t.get("name"):
                        translations[code] = {"name": t["name"], "lang_id": lang_id}
                    img_url = get_yugioh_image_url(t)
            ...
            for code in image_codes:
                ...
                if image_data is None:
                    image_url_fallback = img_url_to_dl.replace(
                        "https://images.ygoprodeck.com/images/cards/",
                        "https://ygoprodeck.com/pics/"
                    )
                    if image_url_fallback != img_url_to_dl:
                        image_data = download_file(image_url_fallback)

                if image_data is None:
                    img_results.append(f"{code}=—")
                    continue

                original_name = f"{passcode}_{code}.jpg"
