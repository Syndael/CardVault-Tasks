...
def get_scryfall_sets(api_base):
    result = fetch_json(f"{api_base}/sets")
    return (result or {}).get("data", [])


def get_scryfall_set_lang(api_base, lang, set_code):
    return fetch_json(f"{api_base}/sets/{set_code}?locale={lang}")
...
def parse_migration_languages(migration_languages):
    ...
    for item in migration_languages.strip(";").split(";"):
        ...
        scryfall_lang.append(lang)
        db_lang[lang] = country

    return scryfall_lang, db_lang
