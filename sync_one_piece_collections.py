...
PARAM_KEY_API_BASE = "sync.one-piece.collections.api.base"
PARAM_KEY_CAR_TYPE = "sync.one-piece.collections.card.type"
PARAM_KEY_MIG_LANG = "sync.one-piece.collections.migration.languages"
...
def get_api_sets(api_base):
    result = fetch_json(f"{api_base}/api/allSets/")
...
def create_collection(card_type_id, set_id, release_date):
    return api_request("POST", "collections", {
        "card_type_id": card_type_id, "code": set_id,
        "release_date": release_date or None, "is_manual": False
    })


def upsert_translation(existing_translation, collection_id, lang_id, name):
    payload = {"collection_id": collection_id, "language_id": lang_id, "name": name}
...
def sync():
    ...
    print(f"\n{SEP}")
    print("  Searching One Piece TCG sets via CardVault API")
    print(SEP)
    ...
    _logger = TaskLogger(log_dir, "one_piece_collections")
    ...
    _logger.log("\n  Getting sets...")
    all_sets = get_api_sets(api_base)
    ...
    for i, item in enumerate(all_sets):
        ...
        if collection:
            ...
            collection = create_collection(card_type_id, set_id, None)
    ...
    finalize_log(_logger, "one_piece_collections", _API_ROOT, api_request)
